	...
	function AttachmentList( attributes )
	{
		...
		var draw = function()
		{
			...
			_sortableComponent = new ArrangeableList( {
		...
		var addAttachment = function( id, title, thumbnailURL, iconURL, editURL )
		{
			...
			var model = new AttachmentItem.Model({
					id : id,
					title : title,
			...
			// bb.collection.add(model);
			var attachment = new AttachmentItem({
				model : model
			});
			attachment.setName( _name + "[]" );
			attachment.on( "remove", adjustButton );
			_addButton.getElement().before( attachment.$el );
